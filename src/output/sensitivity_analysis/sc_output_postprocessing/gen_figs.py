...
def sc_convergence(showfig=True, savefig=False):
    ...
    for cv_idx, cv in enumerate(CV_LIST):
        for sc_idx, sc in enumerate(SCP_LIST):
            ...
            ax[0, cv_idx].plot(K_LIST, mean_mISI, figmarks[sc_idx], markevery=3, label=f'$\\langle mISI \\rangle$, ${sc}$ SC points')
            ax[0, cv_idx].set_title(f'$\\alpha = {cv}$%')
            ax[1, cv_idx].plot(K_LIST, var_mISI, figmarks[sc_idx], markevery=3, label=f'$Var(mISI)$, ${sc}$ SC points')
            ax[2, cv_idx].plot(K_LIST, skew_mISI, figmarks[sc_idx], markevery=3, label=f'$Skew(mISI)$, ${sc}$ SC points')
            ax[3, cv_idx].plot(K_LIST, kurt_mISI, figmarks[sc_idx], markevery=3, label=f'$Kurt(mISI)$, ${sc}$ SC points')
...
def anova(showfig=True, savefig=False):
    ...
    for cv_idx, cv in enumerate(CV_LIST):
        ...
        ax[1, cv_idx].legend(['$g_{Na}$ $g_{K}$', '$g_{Na}$ $g_{L}$', '$g_{K}$ $g_{L}$'])
    ...
    if showfig:
        plt.show()
    if savefig:
        fig.savefig(fname=os.path.join('figures', f'{DATA_PATH}_anova.eps'), format='eps', bbox_inches='tight')
